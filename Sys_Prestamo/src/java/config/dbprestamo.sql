...
DELIMITER $$
CREATE PROCEDURE DELETE_USER(IDPERSONAi INTEGER)
BEGIN
START TRANSACTION;
DELETE FROM PERSONA WHERE IDPERSONA=IDPERSONAi;
DELETE FROM USUARIO WHERE IDPERSONA=IDPERSONAi;
COMMIT;
END$$

DELIMITER $$
CREATE PROCEDURE RETURN_EQU(IDEQi INTEGER)
BEGIN
START TRANSACTION;
UPDATE DET_PRESTAMO SET ESTADO=0 WHERE IDDET_EQUIPO=IDEQi;
UPDATE DET_EQUIPO SET ESTADO=1 WHERE IDDET_EQUIPO=IDEQi;
COMMIT;
END$$

DELIMITER $$
CREATE PROCEDURE DELETE_LOAN(IDPRESTAMOi INTEGER)
BEGIN
START TRANSACTION;
DELETE FROM PRESTAMO WHERE IDPRESTAMO=IDPRESTAMOi;
DELETE FROM DET_PRESTAMO WHERE IDPRESTAMO=IDPRESTAMOi;
COMMIT;
END$$
