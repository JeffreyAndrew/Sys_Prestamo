...
function createTableDoc() {var m = '<table id="tabDc" class="table table-bordered table-striped">';m += '<thead>';m += '<tr>';m += '<th><i class="fa fa-user"></i>   Nombres y Apellidos</th>';m += '<th><i class="fa fa-credit-card"></i>   DNI</th>';m += '<th></th>';m += '</tr>';m += '</thead>';m += '<tbody id="datadoc">';m += '</tbody>';m += '</table>';return m;}function dataloan(id) {var url = "loan?mt=list&op=7";var data = "idp=" + id;$.post(url, data, function (objJson) {var lista = objJson.lista;if (lista.length > 0) {$(".dT").attr("class", "hidden");$(".dataLoan").attr("class", "dataLoan");for (var i = 0; i < lista.length; i++) {$("#idc").attr("value", lista[i].persona);$("#ifp").attr("value", lista[i].fprestamo);$("#ihp").attr("value", lista[i].hprestamo);$("#ihl").attr("value", lista[i].hlimite);var date = new Date();var time = date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();var day = parseInt(date.getDate());var month = parseInt(date.getMonth() + 1);if (day < 10) {day.toString();day = "0" + day;}if (month < 10) {month = "0" + month;}var dta = (date.getFullYear() + "-" + month + "-" + day);if (lista[i] !== dta) {$(".pd").attr("class", "panel panel-danger pd");$(".adviceT").attr("class", "callout callout-danger adviceT");} else {if (lista[i].hlimite < time) {$(".pd").attr("class", "panel panel-danger pd");$(".adviceT").attr("class", "callout callout-danger adviceT");}}}} else {}});}function createTable() {var t = '<table id="TablaDatos" class="table table-condensed daT">';t += '<thead>';t += '<tr>';t += '<th><center><i class="fa fa-laptop"></i>   Marca</center></th>';t += '<th><center>Serie</center></th>';t += '<th><center>Tipo</center></th>';t += '<th><center>Código</center></th>';t += '<th><center>Descripción</center></th>';t += '<th><center>A devolver</center></th>';t += '</tr>';t += '</thead>';t += '<tbody id="datapres">';t += '</tbody>';t += '</table>';return t;}